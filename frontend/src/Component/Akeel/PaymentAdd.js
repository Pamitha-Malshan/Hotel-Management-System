...
export default function Payment(props) {
  ...
  const [payemail, setPaymentEmail] = useState({
    ...
    description: "Thank you for Ordering with Green Hotel",
  });

  useEffect(() => {
    //---------call the get booked rooms function
    getOrderedFood(semail);

    //-------------validate the payment form
    const createFoodForm = $("#pay-form");
  ...
  //---------get Order Food according to the email
  function getOrderedFood(email) {
    console.log(email);
    axios
      .get(`http://localhost:8001/OrderFood/${email}`)
      .then((res) => {
        if (res.data.success) {
          console.log(res.data.existingFoods);
          setOrderFoods(res.data.existingFoods);
        }
      })
      .catch((err) => {
        alert(err.message);
  ...
  const buyNow = async (e) => {
    ...
    const newPayment = {
      ...
      cardExpireDate,
      cvv,
      total: orderFoods.total,
    };

    await axios
      .post("http://localhost:8001/payFood", newPayment)
      .then(() => {
